...
def old_lc_maker(star='unknown', file_path=os.getcwd()+'/', 
...
def lc_maker(star='unknown', file_path=os.getcwd()+'/', save_path=os.getcwd()+'/lightcurves/', bin_time=1., qual_check=True, plot=True):
...
#    rootnames = np.unique(rootnames)
    ...
    #make arrays to store combined lightcurve in 
    #removing this to make it easier to work with more than one star at a time
    #all_time = np.array([], dtype=float)
    #all_counts = np.array([], dtype=float)
    #all_error = np.array([], dtype =float)

    for rootname, band, exp_time in zip(rootnames, bands, exp_times):
